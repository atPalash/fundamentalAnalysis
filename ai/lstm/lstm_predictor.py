...
class LstmPredictor:
    ...
    def build_model(self):
        ...
        try:
            ...
            for stock in self.selected_stocks:
                ...
                model.fit(x_train, y_train, epochs=self.epochs, callbacks=[self.terminate])
    ...
    def predict(self, current_data: pandas.DataFrame, stock_ns: str):
        """
        ...
        """
        try:
            dataset_test = current_data
            dataset_test = np.array(dataset_test).reshape(-1, 1)
            dataset_test = self.scaler.fit_transform(dataset_test)
            x_test, y_test = self.__create_dataset(dataset_test)
            x_test = np.reshape(x_test, (x_test.shape[0], x_test.shape[1], 1))

            if stock_ns not in self.stock_models.keys():
                self.stock_models[stock_ns] = tf.keras.models.load_model(f'{self.model_save_path}/{stock_ns}.h5')
            prediction = self.stock_models[stock_ns].predict(x_test)
            prediction = self.scaler.inverse_transform(prediction)
            y_test_scaled = self.scaler.inverse_transform(y_test.reshape(-1, 1))

            x = numpy.arange(0, len(prediction))
            y = numpy.array(prediction)
            ...
            if self.debug:
                ...
                _, trend_y = abline(z[0], z[1])
                plt.plot(trend_y, color='blue', label='trend')

                plt.legend()
                plt.title(stock_ns)
                plt.show()

            return prediction, z[0][0]
        except Exception as e:
            raise
    ...
    def __build_neural_layer(shape):
        ...
        model.add(CuDNNLSTM(units=96, return_sequences=True, input_shape=(shape, 1)))
